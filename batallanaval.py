#Se nos pide crear un juego donde en una matriz de 5 x 5 se coloquen 3 números 1 que representen barcos y se ubiquen al azar.Los demás esapcios serán ocupados por 0 que representarán  el agua.
...
import numpy as np 
# numpy es una librería que proporciona herramientas para manejar matrices entre otras cosas. 
...
def colocar_barcos(tablero):#Defiimos la función colocar_barcos.(tablero) es la variable que representa
    ...
    while barcos_colocados < 3:#Continúa hasta que la cantidad de barcos sea 3.
        x = np.random.randint(0, 5)#Ésta función de NumPy nos genera números aleatorios enteros que se asignan a x. Los parámetros van de 0 (primero) hasta 5(excluído). O sea que x recibe enteros del 0 al 4.
...
def jugar():
    ...
    while barcos_restantes > 0:#Continúa el juego mientras queden barcos
        ...
        if verificar_disparo(tablero, x, y):#Verifica si el disparo fue un acierto.
            ...
            print(f"Barcos restantes: {barcos_restantes}") #Informa al usuario sobre el número de barcos restantes.
